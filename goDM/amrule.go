				...
				if val.Interface().(*dm.DmClob) == nil {
					...
					if err == nil {
						...
						if reflect.TypeOf(res).Field(i).Name == "RULE_JSON" {
							...
							if err != nil {
								log.Printf("解析rule_json出错：err %v \n", err)
							}
						}
					} else if err == io.EOF {
						log.Printf("规则读取Clob结束：%v\n", err)
					} else {
						log.Printf("规则数据clob非空解析报错：%v\n", err)
					}
					// log.Println(record)
				}
			}
...
func appendVals(rec string, val string) string {
	/**
	增加特殊字段判断逻辑，如果有源数据有单引号包裹的字段数据，使用两个单引号替换
	*/
...
查询规则集历史版本ID
*/
// type amhistset struct {
// 	ID string
// }

func querHistsetId(setId string, histVersion string) (hsid string, err error) {
	/*提取字段值*/
	var ID string
...
查询规则集ID
*/
func querySetId(setName string) (sid string, err error) {
	var ID string
...
func insertAmRule(sqlfile string, targetHistId string, srcHistId string) error {
	...
	log.Println("Insert AmRule succeed")
	return nil
}
