...
func main() {
	...
	if db, err = connect(driverName, dataSourceName); err != nil {
		fmt.Printf("连接 %s 失败.：%v", dataSourceName, err)
	}
	/**
	  从源端导出目标数据逻辑代码：
	*/
	...
	*/
	if ParamsMp["database"] == "target" {
		id, err = querySetId(ParamsMp["set_name"])
		if err != nil {
			fmt.Printf("查询目标规则集ID异常 %v \n", err)
		}
		fmt.Printf("目标规则集ID： %v \n", id)

		hid, err = querHistsetId(id, ParamsMp["set_hist_version"])
		if err != nil {
			fmt.Printf("查询历史版本规则集ID异常 %v \n", err)
		}
		fmt.Printf("目标规则集关联历史版本ID： %v \n", hid)
		//读取SQL文件，加载sql语句，插入规则表。
		if err = insertAmRule(ParamsMp["sqlfile"], hid, ParamsMp["srcHistId"]); err != nil {
			fmt.Printf("插入规则异常 %v \n", err)
		}
		fmt.Println("Rules imported succeed")
	}
...
// 			return err
// 		}
// 		fmt.Printf("insert into am_rule (\"%v\",\"%v\",\"%v\");", rule_no, rule_name, rule_json)
// 	}
// 	return nil
// }
